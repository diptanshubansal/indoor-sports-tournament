...
// Helper for dynamic participant game status
const getParticipantGameStatus = async (playerID, tournamentId, gameName) => {
  const participant = await Participant.findOne({ participantId: playerID });
  if (!participant) return null;

  const enrolled = participant.enrolledGames.includes(gameName);

  const baseResult = {
    gameName,
    enrolled,
    tournamentId,
    fixtureStatus: 'Not Scheduled',
  ...
  const game = await TournamentGame.findOne({ tournamentId, gameName });
  if (!game) {
    return baseResult;
  }

  const hasFixtures = await Round.exists({ tournamentGameId: game._id });
  if (!hasFixtures) {
    return baseResult;
  }

  baseResult.fixtureStatus = 'Scheduled';

  if (game.champion === playerID) {
    baseResult.fixtureStatus = 'Champion';
    baseResult.resultStatus = 'Champion';
    baseResult.round = game.currentRound || 1;
    return baseResult;
  }
  if (game.runnerUp === playerID) {
    baseResult.fixtureStatus = 'Runner-Up';
    baseResult.resultStatus = 'Runner-Up';
    baseResult.round = game.currentRound || 1;
    return baseResult;
  }
  ...
  const byeRounds = await Round.find({
    tournamentGameId: game._id,
    byePlayerId: playerID
  });

  const sortedMatches = matches.sort((a, b) => {
    const rA = a.roundId ? a.roundId.roundNumber : 0;
    const rB = b.roundId ? b.roundId.roundNumber : 0;
    return rB - rA;
  });

  const latestMatch = sortedMatches[0];

  const sortedByeRounds = byeRounds.sort((a, b) => b.roundNumber - a.roundNumber);
  const latestByeRound = sortedByeRounds[0];

  const latestMatchRound = latestMatch && latestMatch.roundId ? latestMatch.roundId.roundNumber : 0;
  const latestByeRoundNum = latestByeRound ? latestByeRound.roundNumber : 0;

  const latestPlayerRound = Math.max(latestMatchRound, latestByeRoundNum);
  baseResult.round = latestPlayerRound || game.currentRound || 1;

  if (latestPlayerRound > 0) {
    if (latestMatchRound >= latestByeRoundNum && latestMatch) {
      const opponentId = latestMatch.player1Id === playerID ? latestMatch.player2Id : latestMatch.player1Id;
      let opponentName = 'TBD';
      if (opponentId) {
        const opp = await Participant.findOne({ participantId: opponentId });
        opponentName = opp ? opp.name : opponentId;
      }
      baseResult.opponent = opponentId ? `${opponentName} (${opponentId})` : 'TBD';

      const isCompleted = ['completed', 'walkover', 'disqualified', 'withdrawn'].includes(latestMatch.status);
      if (isCompleted) {
        if (latestMatch.winnerId === playerID) {
          baseResult.resultStatus = 'Won';
          baseResult.fixtureStatus = baseResult.opponent;
        } else {
          baseResult.resultStatus = 'Eliminated';
          baseResult.fixtureStatus = 'Eliminated';
        }
      } else {
        baseResult.resultStatus = 'Pending';
        baseResult.fixtureStatus = baseResult.opponent;
      }
    } else if (latestByeRound) {
      baseResult.byeStatus = 'Bye';
      baseResult.fixtureStatus = 'Bye - Advanced';
      baseResult.resultStatus = 'Advanced';
    }
  }
...
// @route   GET /api/participants/my-games
// @desc    Get current participant's enrolled games status for a tournament
// @access  Private (Participant)
router.get('/my-games', protect, authorize('participant'), async (req, res) => {
  try {
    ...
    const ALLOWED_GAMES = ['Chess', 'Carrom', 'Table Tennis', 'Ludo', 'Skipping', 'Spoon Race', 'BGMI'];
    const results = [];

    for (const gameName of ALLOWED_GAMES) {
      const statusObj = await getParticipantGameStatus(playerID, tournamentId, gameName);
      if (statusObj) {
